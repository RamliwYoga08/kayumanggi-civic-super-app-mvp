...
import type { Candidate, CivicEvent, CivicIssue, CivicProject, CivicResource, Conversation, Group, Job, LostFoundReport, MarketplaceListing, Message, NewsArticle, Poll, Post, Profile } from '@/types/domain';
...
export async function getPosts(): Promise<Post[]> {
  const { data, error } = await supabase.from('posts').select('id,author_id,body,kind,visibility,created_at,profile:profiles!posts_author_id_fkey(id,full_name,username,avatar_url,is_verified),post_reactions(user_id,reaction),comments(id,body,created_at,profile:profiles!comments_author_id_fkey(id,full_name,avatar_url))').order('created_at', { ascending: false }).limit(40);
  if (error) throw error; return (data || []) as unknown as Post[];
}

export async function createPost(body: string, kind = 'civic') {
  const author_id = await uid(); const { error } = await supabase.from('posts').insert({ author_id, body: body.trim(), kind, visibility: 'public' }); if (error) throw error;
}
...
export async function createListing(input:{title:string;description:string;price:number;category:string;location?:string}) { const seller_id=await uid(); const {error}=await supabase.from('marketplace_listings').insert({...input,seller_id,status:'active'}); if(error)throw error; }
...
export async function getConversations(): Promise<Conversation[]> { const user_id=await uid(); const {data,error}=await supabase.from('conversations').select('id,title,kind,updated_at,conversation_members!inner(user_id)').eq('conversation_members.user_id',user_id).order('updated_at',{ascending:false}); if(error)throw error; return (data||[]) as unknown as Conversation[]; }
