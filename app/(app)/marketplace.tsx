...
import { ActionPill, CircleButton, PageTitleBar, SearchBox, SidebarItem } from '@/components/SocialUI';
...
export default function MarketplaceScreen() {
  ...
  return <View style={{ flex: 1, flexDirection: 'row', backgroundColor: theme.background }}>
    {isDesktop ? <View style={{ width: 330, padding: 14, backgroundColor: theme.surface, borderRightWidth: 1, borderRightColor: theme.border, gap: 5 }}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}><Title size={24}>Marketplace</Title><CircleButton icon="⚙" label="Marketplace settings" /></View>{controls}<SidebarItem icon="▦" label="Browse all" active /><SidebarItem icon="♢" label="Notifications" /><SidebarItem icon="✉" label="Inbox" /><SidebarItem icon="☆" label="Saved items" /><Button onPress={() => setShowCreate(true)} style={{ marginTop: 8 }}>＋ Create new listing</Button><View style={{ height: 1, backgroundColor: theme.border, marginVertical: 10 }} /><Title size={14}>Location</Title><Text style={{ color: theme.info, fontSize: 12, fontWeight: '800' }}>Your listing locations</Text></View> : null}
    <View style={{ flex: 1 }}>
      {!isDesktop ? <><PageTitleBar title="Marketplace" right={<View style={{ flexDirection: 'row', gap: 7 }}><CircleButton icon="⌕" label="Search" /><CircleButton icon="＋" label="Sell" onPress={() => setShowCreate(true)} /></View>} /><View style={{ paddingHorizontal: 12 }}>{controls}</View></> : null}
