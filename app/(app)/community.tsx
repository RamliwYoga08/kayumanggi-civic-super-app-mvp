...
import { getGroups, getIncomingFriendRequests, getPeople, joinGroup, respondToFriendRequest, sendFriendRequest } from '@/services/api';
...
type CommunityTab = 'requests'|'suggestions'|'groups';
...
export default function CommunityScreen() {
  ...
  const tabs: { id: CommunityTab; label: string; icon: string }[] = [{ id: 'requests', label: 'Friend requests', icon: '◉' }, { id: 'suggestions', label: 'Suggestions', icon: '＋' }, { id: 'groups', label: 'Groups', icon: '◎' }];

  return <View style={{ flex: 1, flexDirection: 'row', backgroundColor: theme.background }}>
    {isDesktop ? <View style={{ width: 330, padding: 10, backgroundColor: theme.surface, borderRightWidth: 1, borderRightColor: theme.border }}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 6 }}><Title size={24}>Community</Title><CircleButton icon="⚙" label="Community settings" /></View><SidebarItem icon="⌂" label="Home" /><SidebarItem icon="◉" label="Friend requests" detail={`${requests.data?.length || 0} pending`} active={tab === 'requests'} onPress={() => setTab('requests')} /><SidebarItem icon="＋" label="Suggestions" active={tab === 'suggestions'} onPress={() => setTab('suggestions')} /><SidebarItem icon="◎" label="Groups" active={tab === 'groups'} onPress={() => setTab('groups')} /></View> : null}
    <View style={{ flex: 1 }}>
